...
import { RYNOTE_BANNER_FILE, RYNOTE_BANNER_URL, RYNOTE_INVITE, RYNOTE_SUPPORT } from '../../utilities/Links.js'
...
export default class {
  async execute(client: Manager, message: Message) {
    ...
    if (message.content.match(mention) && checkBlackklistRes) {
      const lang = guildModel
      const desc =
        `${client.i18n.get(lang, 'event.message', 'mention_what_is', {
          username: client.user!.username,
        })}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_desc', {
          username: client.user!.username,
        })}\n\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_can_do')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_music')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_player')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_filters')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_playlists')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_utility')}\n\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_why', {
          username: client.user!.username,
        })}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_why_audio')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_why_fast')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_why_rich')}\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_why_support')}\n\n` +
        `${client.i18n.get(lang, 'event.message', 'mention_footer', { prefix: PREFIX })}`
      ...
      const container = {
        ...
        components: [
          {
            type: 10,
            content: `${client.i18n.get(lang, 'event.message', 'mention_welcome', {
              username: client.user!.username,
    ...
    const command =
      client.prefixCommands.get(cmd) ||
      client.prefixCommands.get(client.aliasesPrefix.get(cmd) as string)
    if (!command) return
    ...
    if (command.accessableby.includes(Accessableby.Admin) && !userPerm.admin)
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(language, 'error', 'user_no_perms', { perm: 'rynote@admin' })}`
            )
            .setColor(client.color),
        ],
      })

    if (command.accessableby.includes(Accessableby.Dev) && !userPerm.owner)
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${client.i18n.get(language, 'error', 'user_no_perms', { perm: 'rynote@dev' })}`
            )
            .setColor(client.color),
        ],
      })
