...
export class Manager extends Client {
  ...
  public prefixCommands: DJSCollection<string, Command>
  ...
  public aliasesPrefix: Collection<string>
  ...
  constructor(
    ...
    const raw = this.config.bot.EMBED_COLOR || '#2b2d31'
    this.color = parseInt(String(raw).replace('#', ''), 16) as ColorResolvable
  ...
  public start() {
    this.logger.info('ClientManager', `Booting rynote...`)
    ...
    this.loginWithRetry(this.config.bot.TOKEN, 0)
  }

  private loginWithRetry(token: string, attempt: number) {
    const MAX_RETRIES = 5
    ...
    super.login(token).catch((err) => {
      if (attempt < MAX_RETRIES) {
        this.logger.warn(
          'ClientManager',
          `Login failed (attempt ${attempt + 1}/${MAX_RETRIES}): ${err.message}. Retrying in ${RETRY_DELAY / 1000}s...`
        )
        setTimeout(() => this.loginWithRetry(token, attempt + 1), RETRY_DELAY)
      } else {
        this.logger.error('ClientManager', `Login failed after ${MAX_RETRIES} attempts. Exiting.`)
        process.exit(1)
      }
    })
  }
