...
export class RainlinkPlugin extends Plugin {
  ...
  private async search(
    ...
    // Always try alternative sources first (scsearch, spsearch, etc.)
    const searchRes = await this.searchEngine(query, options)
    if (searchRes.tracks.length !== 0) return searchRes
