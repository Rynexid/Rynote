...
const filterSelect = (client: Manager, disable?: boolean, language = 'en') => {
  const options = client.selectMenuOptions.map((o: any) => {
    const label = o.data.label
    const value = o.data.value
    const description =
      value === 'clear'
        ? client.i18n.get(language, 'command.filter', 'filter_reset')
        : `${label} ${client.i18n.get(language, 'command.filter', 'filter_desc')}`
  ...
  return new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId('filter')
      .setPlaceholder(client.i18n.get(language, 'command.filter', 'filter_placeholder'))
      .addOptions(options)
      .setDisabled(disable)
  )
}
