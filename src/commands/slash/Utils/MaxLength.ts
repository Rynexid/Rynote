  ...
  public async execute(client: Manager, handler: CommandHandler) {
    ...
    if (time == 'none' || time == '0:00') {
      ...
      return handler.editReply({
        flags: 32768,
        components: buildV2({
          description: `${client.i18n.get(handler.language, 'command.utils', 'ml_remove')}`,
          color: client.color,
        }),
      } as any)
    }
    ...
    if (!time_regex.test(time))
      return handler.editReply({
        flags: 32768,
        components: buildV2({
          description: `${client.i18n.get(handler.language, 'command.utils', 'ml_invalid')}`,
          color: client.color,
        }),
      } as any)
    else {
    ...
    return handler.editReply({
      flags: 32768,
      components: buildV2({
        description: client.i18n.get(handler.language, 'command.utils', 'ml_set', { time }),
        color: client.color,
      }),
    } as any)
  }
